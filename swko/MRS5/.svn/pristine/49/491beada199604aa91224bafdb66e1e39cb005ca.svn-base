...
public class Rental {
    ...
    /** The maximum amount of movies, one user can rent at a time. */
     ...
     * @throws IllegalArgumentException if the user has already rented the maximum amount of movies.
     * @throws MovieRentalException if the user is not old enough for the movie
     * @throws MovieRentalException if the user is not old enough for the movie
     */
    public Rental(IUser aUser, Movie aMovie, int nofRentalDays) {
        if (isValidUser(aUser) && isValidMovie(aMovie) && isValidRentalDays(nofRentalDays)) {
            ...
            if (aUser.getRentals().size() == maxMovies) {
                throw new IllegalStateException("You can only rent " + maxMovies + " movies at a time.");
            }
            if (!isUserOfAge(aMovie, aUser)) {
                throw new MovieRentalException("user not of age");
            }
     ...
     */
    private static boolean isUserOfAge(Movie m, IUser u) {
        Calendar today = Calendar.getInstance();
        Calendar birthdate = u.getBirthdate();
        int ageRating = m.getAgeRaiting();
        birthdate.add(Calendar.YEAR, ageRating);
        boolean re = birthdate.compareTo(today) <= 0;
        birthdate.add(Calendar.YEAR, -ageRating);
        return re;
    }

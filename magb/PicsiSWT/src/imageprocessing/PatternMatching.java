...
public class PatternMatching implements IImageProcessor {
    ...
    public Image run(Image input, int imageType) {
        ...
        //preprocessing: müR, sigmaR, müR2 (Mittelwert von R^2) bestimmen
        ...
        for(int s=0;s<inData.height-pattern.height;s+=stepSize){
            for(int r=0;r<inData.width-pattern.width;r+=stepSize){
                ...
                myI /=size;
                double c = (sumIR-size*myI*myR)/(Math.sqrt(sumI2-size*myI)*sigmaR*sqrtSize);
                if(c > cMax){
                    cMax = c;
                    bestPosX = r;
                    bestPosY = s;
